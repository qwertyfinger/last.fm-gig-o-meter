...
public class DatabaseHelper {
    ...
    public Observable<Void> addScrobbleData(Map<ArtistDb, List<TrackDb>> scrobbleData) {
        return Observable.create(subscriber -> {
            addArtists(scrobbleData.keySet())
                    .retry(2)
                    .subscribe(aVoid -> {}, subscriber::onError, () ->
                            addTracks(scrobbleData.values())
                                    .retry(2)
                                    .subscribe(bVoid -> {}, subscriber::onError, subscriber::onCompleted));
        });
    }

    public Observable<List<ArtistDb>> getAllArtists() {
        return mDb.createQuery(ArtistContract.TABLE_NAME,
                "SELECT  * FROM " + ArtistContract.TABLE_NAME + " ORDER BY "
                    + ArtistContract.COLUMN_SYNC_STATUS + " DESC, "
                    + ArtistContract.COLUMN_COMPATIBILITY + " DESC, "
                    + ArtistContract.COLUMN_PLAYCOUNT + " DESC, "
                    + ArtistContract.COLUMN_NAME + " ASC")
                .mapToList(ArtistContract::parseCursor);
    }
    ...
    public Observable<List<TrackDb>> getTracksByArtist(String artistName) {
        return mDb.createQuery(TrackContract.TABLE_NAME, "SELECT  * FROM " + TrackContract.TABLE_NAME
                        + " WHERE " + TrackContract.COLUMN_ARTIST + " = ?", artistName)
