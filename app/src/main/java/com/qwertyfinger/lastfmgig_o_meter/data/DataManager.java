...
public class DataManager {
            ...
            mLastFmService.checkUser(BuildConfig.LAST_FM_API_KEY, username)
                    ...
                    .subscribe(response -> {
                        ...
                        if (e instanceof HttpException) {
                            ...
                            if (httpException.response().code() == 400) subscriber.onNext(false);
        ...
        return Observable.create(subscriber -> {
            ...
            Observable.from(artists)
                    ...
                    .subscribe(tracks -> {
                        ...
                        syncWithSetilstFm(scrobbleData, artists.size())
                                .subscribe(aVoid -> {}, subscriber::onError);
    ...
    public Observable<Void> syncData() {
        return Observable.create(subscriber -> {
            ...
            getLastFmInfo()
                    .subscribe(tracks::addAll, subscriber::onError);
            ...
            syncWithSetilstFm(scrobbleData, mPrefHelper.getArtistsLimit())
                    .subscribe(aVoid -> {}, subscriber::onError);
        ...
        return Observable.create(subscriber -> {
            ...
            mLastFmService.getTopTracks(BuildConfig.LAST_FM_API_KEY, username, 1, 1)
                    ...
                    .subscribe(stats -> {
                        ...
                        double limit;
                        if (totalTracks < 20000) limit = totalTracks * 0.25;
                        else limit = 5000;
                        ...
                        Observable.from(pages)
                                .concatMap(page -> mLastFmService.getTopTracks(BuildConfig.LAST_FM_API_KEY,
                                        username, (int) limit, page))
                                .retry(2)
                                .takeWhile(trackList ->
                                        trackList.getTracks().get(0).getPlaycount() > 2
                                ...
                                .subscribe(trackList -> result.addAll(trackList.getTracks()),
                                        subscriber::onError);
                    }, subscriber::onError);
        ...
        return Observable.create(subscriber -> {
            ...
            mLastFmService.getLovedTracks(BuildConfig.LAST_FM_API_KEY, username, 1, 1)
                    ...
                    .subscribe(stats -> {
                        ...
                        Observable.from(pages)
                                .concatMap(page -> mLastFmService.getLovedTracks(BuildConfig.LAST_FM_API_KEY,
                                        username, 1000, page))
                                .retry(2)
                                .subscribe(trackList -> result.addAll(trackList.getTracks()),
                                        subscriber::onError);
...
//                        .retry(2)
                        .subscribe(firstPage -> {
                            ...
                            if (firstPage.getTotal() > 20 && setlists.size() < 15
...
//                                            TODO: react to error accordingly
                                            Timber.e(e, e.getClass().getCanonicalName() + ", " +
                                                    artist.getName() + ", " + artist.getMbid() + ", first page");
...
//                            TODO: react to error accordingly
                ...
                if (setlists.size() > 2) {
                    ...
                    for (int j = 0; j < artistTracks.size(); j++) {
                        ...
                        track.setProbability((double) track.getArtistPlaycount()
                                / artist.getNmbOfConcerts());
                        track.setWeight((track.getUserPlaycount() / mPrefHelper.getCoefficient())
                                * track.getProbability());
                        if (track.isLoved()) {
    ...
    private void createPicasso() {
        ...
        Picasso.setSingletonInstance(new Picasso.Builder(Utils.getAppContext())
